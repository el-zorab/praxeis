...
struct madt_record_lapic *lapic_records[SMP_MAX_CORES];
uint64_t lapic_counter;

struct madt_record_ioapic *ioapic_records[IOAPIC_MAX_RECORDS];
...
void madt_init(void) {
    ...
    uint64_t size = madt->header.length - sizeof(struct madt);
    struct madt_record_header *record_header;
    for (uint64_t i = 0; i < size; i += record_header->length) {
        record_header = (struct madt_record_header*) (uintptr_t) (madt->records + i);
        switch (record_header->type) {
            case MADT_TYPE_LAPIC:
                if (lapic_counter > SMP_MAX_CORES) {
                    panic("System has more LAPICs than the maximum (%llu)", true, SMP_MAX_CORES);
                }
                lapic_records[lapic_counter++] = (struct madt_record_lapic*) record_header;
                break;
            case MADT_TYPE_IOAPIC:
                if (ioapic_counter > IOAPIC_MAX_RECORDS) {
                    panic("System has more IOAPICs than the maximum (%llu)", true, IOAPIC_MAX_RECORDS);
                }
                ioapic_records[ioapic_counter++] = (struct madt_record_ioapic*) record_header;
                break;
        }
    }
